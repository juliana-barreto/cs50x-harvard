...
bool check(char *word) {
    node *cursor = root;
    // Traverse the trie according to the characters in word
    for (int i = 0; word[i] != '\0'; i++) {
        // Calculate index in children array
        int index = tolower(word[i]) - 'a';
        // If there is no node, word is not in trie
        if (cursor->children[index] == NULL) {
            return false;
        }
        cursor = cursor->children[index];
    }
    // After traversing all characters, check if we are at a word
